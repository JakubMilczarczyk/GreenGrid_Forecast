...
with DAG(
    ...
    featch_entsoe_data = BashOperator(
        ...
        bash_command="python /opt/airflow/dags/utils/extract/universal_entsoe_data_featcher.py"
    )

    extract_wether_data = BashOperator(
        task_id="run_weather_fetcher",
        bash_command="python /opt/airflow/dags/utils/extract/featch_weather_data.py"
    )

    transform_entsoe_forecast_data = BashOperator(
        task_id="run_entsoe_forecast_transformer",
        bash_command="python /opt/airflow/dags/utils/transform/entsoe_forecast_parser.py"
    )

    transform_entsoe_generation_data = BashOperator(
        task_id="run_entsoe_generation_transformer",
        bash_command="python /opt/airflow/dags/utils/transform/entsoe_generation_parser.py"
    )

    merge_faetures_data = BashOperator(
        task_id="merge_features_data",
        bash_command="python /opt/airflow/dags/utils/transform/train_features_data_merger.py"
    )
